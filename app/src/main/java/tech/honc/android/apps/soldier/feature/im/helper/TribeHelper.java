...
public class TribeHelper {
  ...
  public static IYWTribeService getTribeService() {
    final YWIMKit imKit = LoginHelper.getInstance().getYWIMKit();
    if (imKit != null) {
   ...
   *
   */
  public static void createTribe(YWTribeType tribeType) {
    ...
    if (tribeType == YWTribeType.CHATTING_GROUP) {
      ...
      final YWIMCore core = LoginHelper.getInstance().getYWIMKit().getIMCore();
  ...
  public static void getAllTribeFromServer_Sample() {
    ...
    tribeService.getAllTribesFromServer(new MyCallback() {
      ...
      @Override public void onSuccess(Object... arg0) {
        // 返回值为列表
        @SuppressWarnings("unchecked") List<YWTribe> tribeList = (List<YWTribe>) arg0[0];
        tribeList.size();
      }
    });
  }
  ...
  public static void getMembersFromServer_Sample(long tid) {
    ...
    tribeService.getMembersFromServer(new MyCallback() {
      ...
      @Override public void onSuccess(Object... arg0) {
        @SuppressWarnings("unchecked") List<YWTribeMember> memberList =
            (List<YWTribeMember>) arg0[0];
        memberList.size();
      }
    }, tid);
  }
