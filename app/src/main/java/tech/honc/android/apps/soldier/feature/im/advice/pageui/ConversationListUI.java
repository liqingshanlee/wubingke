...
public class ConversationListUI extends IMConversationListUI {
  ...
  private static final String TAG = "ConversationListUI";
   ...
   * 返回会话列表的自定义标题
   */
  @Override public View getCustomConversationListTitle(final Fragment fragment,
    ...
    RelativeLayout customView =
        (RelativeLayout) inflater.inflate(R.layout.im_custom_conversation_title_bar,
            new RelativeLayout(context), false);
    customView.setBackgroundColor(Color.parseColor("#1A9924"));
    TextView title = (TextView) customView.findViewById(R.id.title_txt);
    final YWIMKit mIMKit = LoginHelper.getInstance().getYWIMKit();

    title.setText(mIMKit.getIMCore().getShowName());
    title.setTextColor(Color.WHITE);
    final String loginUserId = LoginHelper.getInstance().getYWIMKit().getIMCore().getLoginUserId();
    final String appKey = LoginHelper.getInstance().getYWIMKit().getIMCore().getAppKey();
      ...
      @Override public void onClick(View arg0) {
        ...
        if (TextUtils.isEmpty(loginUserId) || TextUtils.isEmpty(appKey)) {
          ...
          Toast.makeText(context,
              "click (" + " userid = " + loginUserId + " ,appKey = " + appKey + " )",
   ...
   */
  @Override public void setCustomTitleProgressBar(Fragment fragment, View mCustomTitleView,
      boolean isVisible) {

  }

  @Override public boolean needHideTitleView(Fragment fragment) {
    return true;
  }

  @Override public boolean needHideNullNetWarn(Fragment fragment) {
    return false;
  }
   ...
   * 是否支持下拉刷新
   */
  @Override public boolean getPullToRefreshEnabled() {
    return true;
  }
   ...
   * 返回默认的群头像
   */
  @Override public int getTribeConversationHead(Fragment fragment, YWConversation conversation) {
    return R.mipmap.ic_default_trib;
  }

  /**
   * 返回自定义置顶回话的背景色(16进制字符串形式)
   */
  @Override public String getCustomTopConversationColor() {
    return "#e1f5fe";
  }

  @Override public boolean enableSearchConversations(Fragment fragment) {
    return false;
  }
   ...
   * @param convertView {@link android.widget.BaseAdapter#getView(int, View, ViewGroup)}的第二个参数
   * @param parent {@link android.widget.BaseAdapter#getView(int, View, ViewGroup)}的第三个参数
   */
  @Override public View getCustomView(Context context, YWConversation conversation,
      View convertView, ViewGroup parent) {
    LayoutInflater inflater = LayoutInflater.from(context);
    ViewHolder viewHolder = null;
    if (convertView == null) {
      viewHolder = new ViewHolder();
      convertView = inflater.inflate(R.layout.im_conversation_custom_view_item, parent, false);
  ...
  /**
   * {@link ConversationListUI#getCustomView(Context, YWConversation, View,
   * ViewGroup)}中使用到的ViewHolder示例
   */
   ...
   * 会话列表Activity创建事件
   */
  @Override public void onActivityCreated(Bundle savedInstanceState, Fragment fragment) {
   ...
   * 会话列表onResume事件
   */
  @Override public void onResume(Fragment fragment) {
   ...
   * @return empty view
   */
  @Override public View getCustomEmptyViewInConversationUI(Context context) {
   ...
   * @return 资源Id
   */
  @Override public int getCustomBackgroundResId() {
    return 0;
  }
   ...
   * @return 种类数
   */
  @Override public int getCustomItemViewTypeCount() {
    return typeCount;
  }
   ...
   * 自定义item的viewType
   */
  @Override public int getCustomItemViewType(YWConversation conversation) {
   ...
   */
  @Override public View getCustomItemView(Fragment fragment, YWConversation conversation,
      View convertView, int viewType, YWContactHeadLoadHelper headLoadHelper, ViewGroup parent) {
    if (viewType == type_1) {
      ViewHolder1 holder = null;
      if (convertView == null) {
        LayoutInflater inflater = LayoutInflater.from(fragment.getActivity());
        holder = new ViewHolder1();
        convertView = inflater.inflate(R.layout.im_custom_conversation_item_1, parent, false);
      ...
      if (conversation.getConversationBody() instanceof YWCustomConversationBody) {
        YWCustomConversationBody body =
            (YWCustomConversationBody) conversation.getConversationBody();
        String conversationId = body.getIdentity();
        if (conversationId.equals(Constant.SYSTEM_TRIBE_CONVERSATION)) {
          headLoadHelper.setCustomHeadView(holder.head, R.mipmap.ic_sys_tribe, null);
          name = "群系统消息";
        } else if (conversationId.equals(Constant.SYSTEM_FRIEND_REQ_CONVERSATION)) {
          headLoadHelper.setCustomHeadView(holder.head,R.mipmap.ic_default_trib, null);
          name = "联系人系统消息";
        } else {
    ...
    return super.getCustomItemView(fragment, conversation, convertView, viewType, headLoadHelper,
        parent);
  }

  private Map<String, CharSequence> mSmilyContentCache = new HashMap<String, CharSequence>();
  //表情的本地缓存，加速读取速度用
  ...
  private void setSmilyContent(Context context, String content, ViewHolder2 holder) {
    ...
    if (content == null || holder.content.getPaint() == null) {
      ...
      if (charSequence != null) {
        ...
        CharSequence smilySpanStr =
            smilyManager.getSmilySpan(context, content, defaultSmilySize, false);
      ...
      if (charSequence != null) {
        ...
        CharSequence text = TextUtils.ellipsize(content, holder.content.getPaint(), contentWidth,
            holder.content.getEllipsize());
        CharSequence smilySpanStr =
            smilyManager.getSmilySpan(context, String.valueOf(text), defaultSmilySize, false);
        mSmilyContentCache.put(content, smilySpanStr);
        holder.content.setText(smilySpanStr);
      }
    }
  ...
  private void initSmilyManager(Context context) {
    if (smilyManager == null) {
      ...
      defaultSmilySize =
          (int) context.getResources().getDimension(R.dimen.aliwx_smily_column_width);
      ...
      contentWidth = width
          ...
          .getDimensionPixelSize(R.dimen.aliwx_message_content_margin_right);
    }
  }
