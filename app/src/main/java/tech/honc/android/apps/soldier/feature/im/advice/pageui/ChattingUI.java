...
import tech.honc.android.apps.soldier.feature.im.helper.UIHelper;
import tech.honc.android.apps.soldier.feature.im.ui.activity.ContactSettingActivity;
import tech.honc.android.apps.soldier.feature.im.ui.activity.TribeInfoActivity;

/**
 * 聊天界面自带提供两种主题的自定义供用户方便的使用，用户可以通过{@link UIHelper｝中 实现 AdviceBinder.bindAdvice(PointCutEnum.CHATTING_FRAGMENT_UI_POINTCUT,
...
public class ChattingUI extends IMChattingPageUI {
   ...
   */
  @Override public int getMsgBackgroundResId(YWConversation conversation, YWMessage message,
      boolean self) {
    int msgType = message.getSubType();
    if (msgType == YWMessage.SUB_MSG_TYPE.IM_TEXT || msgType == YWMessage.SUB_MSG_TYPE.IM_AUDIO) {
      if (self) {
        return R.drawable.selector_im_right;
      } else {
        return R.drawable.selector_im_left;
      }
  ...
  /**
   * 建议使用{@link #processBitmapOfLeftImageMsg｝和{@link #processBitmapOfRightImageMsg｝灵活修改Bitmap，达到对图像进行［圆角处理］,［裁减］等目的,这里建议return
   * false
   * 设置是否需要将聊天界面的图片设置为圆角
   *
   * @return false: 不做圆角处理
   * <br>
   * true：做圆角处理（重要：返回true时不会做{@link #processBitmapOfLeftImageMsg｝和{@link
   * #processBitmapOfRightImageMsg｝二次图片处理，两者互斥！）
   */

  @Override public boolean needRoundChattingImage() {
    return true;
  }
   ...
   */
  @Override public float getRoundRadiusDps() {
    return 12.6f;
  }
   ...
   * @return 聊天窗口背景，默认不显示
   */
  @Override public int getChattingBackgroundResId() {
    //聊天窗口背景，默认不显示
    return 0;
   ...
   */
  @Override public boolean needHideTitleView(Fragment fragment, YWConversation conversation) {
  ...
  @Override public View getCustomTitleView(final Fragment fragment, final Context context,
    ...
    View view =
        inflater.inflate(R.layout.im_custom_chatting_title, new RelativeLayout(context), false);
    view.setBackgroundColor(Color.parseColor("#1A9924"));
    ...
    if (conversation.getConversationType() == YWConversationType.P2P) {
      ...
      if (!TextUtils.isEmpty(conversationBody.getContact().getShowName())) {
        ...
        IYWContact contact = imKit.getContactService()
            .getContactProfileInfo(conversationBody.getContact().getUserId(),
    ...
    backView.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_arrow_back_white_24dp, 0, 0, 0);
    ...
    if (conversation.getConversationType() == YWConversationType.Tribe) {
      ...
      btn.setOnClickListener(new View.OnClickListener() {
        @Override public void onClick(View v) {
          YWP2PConversationBody pConversationBody =
              (YWP2PConversationBody) conversation.getConversationBody();
          String appKey = pConversationBody.getContact().getAppKey();
          String userId = pConversationBody.getContact().getUserId();
          // TODO: 2016-5-20-0020 联系人信息 
          
          Intent intent =
              ContactSettingActivity.getContactSettingActivityIntent(context, appKey, userId);
      ...
      String feedbackAccount =
          IMPrefsTools.getStringPrefs(IMChannel.getApplication(), IMPrefsTools.FEEDBACK_ACCOUNT,
              "");
      if (!TextUtils.isEmpty(feedbackAccount) && feedbackAccount.equals(
          AccountUtils.getShortUserID(conversation.getConversationId()))) {
        btn.setVisibility(View.GONE);
      }
    }

    //群会话则显示@图标
    //if (YWSDKGlobalConfig.getInstance().enableTheTribeAtRelatedCharacteristic()) {
    //  if (conversation.getConversationBody() instanceof YWTribeConversationBody) {
    ...
    //        bundle.putLong(ChattingDetailPresenter.EXTRA_TRIBEID,
    //            ((YWTribeConversationBody) conversation.getConversationBody()).getTribe()
    //                .getTribeId());
    //        String userId = null;
   ...
   */
  @Override public boolean onImagePreviewTitleButtonClick(Fragment fragment, YWMessage message) {
   ...
   */
  @Override public String getImageSavePath(Fragment fragment, YWMessage message) {
    //        return Environment
   ...
   * @return 0:使用SDK默认提供的
   */
  @Override public int getDefaultHeadImageResId() {
   ...
   * @return 返回要展示的View
   */
  @Override public View getChattingFragmentCustomViewAdvice(Fragment fragment, Intent intent) {
    ...
    if (intent != null && intent.hasExtra("extraTribeId") && intent.hasExtra("conversationType")) {
      ...
      if (tribeId > 0 && conversationType == YWConversationType.Tribe.getValue()) {
        ...
        if (tribe != null
          ...
          lp.setMargins(0, (int) context.getResources().getDimension(R.dimen.title_bar_height), 0,
              0);
          ...
          view.setOnClickListener(new View.OnClickListener() {
            @Override public void onClick(View v) {
              final AlertDialog alertDialog = new WxAlertDialog.Builder(context).setTitle("提示")
   ...
   * @param intent 打开聊天界面Activity的Intent
   */
  @Override public boolean isUseChattingCustomViewAdvice(Fragment fragment, Intent intent) {
    if (intent != null && intent.hasExtra("extraTribeId") && intent.hasExtra("conversationType")) {
  ...
  private void receiveTribeMsg(YWTribe tribe, final View view) {
    LoginHelper.getInstance()
        ...
        .getIMCore()
        .getTribeService()
        .unblockTribe(tribe, new IWxCallback() {
   ...
   */
  @Override public void modifyRightItemParentViewAfterSetValue(YWMessage msg,
      RelativeLayout rightItemParentView, Fragment fragment, YWConversation conversation) {
   ...
   */
  @Override public void modifyLeftItemParentViewAfterSetValue(YWMessage msg,
      RelativeLayout leftItemParentView, Fragment fragment, YWConversation conversation) {
   ...
   */
  @Override public boolean needHideChattingReplyBar() {
    return false;
  }
   ...
   * 是否隐藏底部ChattingReplyBar
   */
  @Override public boolean needHideChattingReplyBar(YWConversation conversation) {
    return false;
  }
   ...
   * false:显示表情按钮
   */
  @Override public boolean needHideFaceView() {
    return false;
  }
   ...
   * false:显示语音发送按钮
   */
  @Override public boolean needHideVoiceView() {
    return false;
  }
   ...
   */
  @Override public int getCustomChattingReplyBarHeight() {
    return 0;
  }
   ...
   * @return 如果返回值小于等于0, 则使用默认值
   */
  @Override public int getCustomChattingInputEditTextHeight() {
    return 0;
  }
   ...
   * 返回表情按钮图标背景资源Id
   */
  @Override public int getFaceViewBgResId() {
    return R.drawable.ic_im_emoji;
  }

  /**
   * 返回"+号"按钮选中图标背景资源Id
   */
  @Override public int getExpandViewCheckedBgResId() {
    return R.drawable.ic_im_add;
  }

  /**
   * 返回"+号"按钮取消选中图标背景资源Id
   */
  @Override public int getExpandViewUnCheckedBgResId() {
    return R.drawable.ic_im_add;
  }
   ...
   * 返回发送按钮背景资源Id
   */
  @Override public int getSendButtonBgId() {
    return 0;
  }
   ...
   * 返回语音按钮图标背景资源Id
   */
  @Override public int getVoiceViewBgResId() {
    return R.drawable.ic_im_voice;
  }

  /**
   * 返回键盘按钮图标资源Id
   */
  @Override public int getKeyboardViewBgResId() {
    return 0;
  }
   ...
   * UI仅显示语音发送
   */
  @Override public boolean onlySupportAudio() {
    return false;
  }
   ...
   */
  @Override public void onSetAudioContentImage(ImageView view, int index, int direction) {
   ...
   */
  @Override public List<YWInputViewPlugin> adjustCustomInputViewPlugins(final Fragment fragment,
      YWConversation conversation, List<YWInputViewPlugin> pluginItems) {
  ...
  @Override public void onInitFinished(IMChattingBizService bizService) {
    if (bizService != null) {
      //建议使用WeakReference方式来保存,避免内存泄漏
      WeakReference<IMChattingBizService> weakReference =
          new WeakReference<IMChattingBizService>(bizService);
   ...
   * @return 颜色资源Id
   */
  @Override public int getCustomTextColor(YWConversation conversation, boolean isSelf, int type) {
    if (type == 1) {
      //普通文字颜色
    } else if (type == 2) {
      //超链接颜色
