...
public class TribeMembersAdapter extends YWAsyncBaseAdapter {
  ...
  @Override public View getView(int position, View convertView, ViewGroup parent) {
    ...
    if (convertView == null) {
      convertView = inflater.inflate(R.layout.im_tribe_members_item, null);
    ...
    if (mList != null) {
      ...
      if (user != null) {
        ...
        mContactHeadLoadHelper.setHeadView(holder.headView, user.getUserId(), user.getAppKey(),
            true);
        holder.nick.setText(
            IMUtility.getTribeShowName(user.getUserId(), user.getAppKey(), user.getTribeNick()));
    ...
    if (convertViewWidth == 0) {
      convertViewWidth = convertView.getMeasuredWidth() == 0 ? convertView.getWidth()
          : convertView.getMeasuredWidth();
    }
    if (convertViewWidth - (holder.headView.getMeasuredWidth()
        + headParams.leftMargin
        + headParams.rightMargin) - (holder.role.getMeasuredWidth()
        + roleParams.leftMargin
        + roleParams.rightMargin) - (nickParams.rightMargin + nickParams.leftMargin) > 0) {
      holder.nick.setMaxWidth(convertViewWidth - (holder.headView.getMeasuredWidth()
          + headParams.leftMargin
          + headParams.rightMargin) - (holder.role.getMeasuredWidth()
          + roleParams.leftMargin
          + roleParams.rightMargin) - (nickParams.rightMargin + nickParams.leftMargin));
    }
    return convertView;
  }
  ...
  public static class ContactImpl implements IYWContact {
    ...
    public ContactImpl(String showName, String userid, String avatarPath, String signatures,
        String appKey) {
    ...
    @Override public String getAppKey() {
      return appKey;
    }

    @Override public String getAvatarPath() {
      return avatarPath;
    }

    @Override public String getShowName() {
      return showName;
    }
